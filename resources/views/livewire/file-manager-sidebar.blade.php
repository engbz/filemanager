<div class="fi-sidebar-nav-filemanager">
    ...
    <nav class="space-y-0.5 px-2">
        ...
        <div x-data="{ showActions: false }" @mouseenter="showActions = true" @mouseleave="showActions = false" class="group">
            <div class="flex w-full items-center gap-1 rounded-lg px-2 py-1.5 text-sm transition-colors hover:bg-gray-100 dark:hover:bg-white/5 {{ $currentPath === null ? 'bg-gray-100 dark:bg-white/5 font-medium' : '' }}">
                ...
                <button wire:click="navigateTo(null)" class="flex items-center gap-2 flex-1 min-w-0 text-left">
                ...
                <div class="relative shrink-0 flex items-center justify-end" style="min-width: {{ $isReadOnly ? '32px' : '40px' }};">
                    ...
                    @if(!$isReadOnly)
                        ...
                        <div class="flex items-center gap-0.5 transition-opacity duration-100" :class="showActions ? 'opacity-100' : 'opacity-0 pointer-events-none'">
                            <button wire:click.stop="openCreateSubfolderDialog(null)" class="p-0.5 rounded hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" title="{{ __('filemanager::messages.add_folder') }}">
    ...
    @if(!$isReadOnly)
        ...
        <x-filament::modal id="sidebar-create-subfolder-modal" width="md">
            <x-slot name="heading">
                {{ __('filemanager::messages.create_new_folder') }}
            </x-slot>
            ...
            <x-slot name="description">
                ...
                @else
                    {{ __('filemanager::messages.create_in_root') }}
                @endif
            </x-slot>

            <form wire:submit="createSubfolder">
                <x-filament::input.wrapper>
                    <x-filament::input type="text" wire:model="subfolderName" placeholder="{{ __('filemanager::messages.folder_name') }}" autofocus/>
                </x-filament::input.wrapper>

                <div class="mt-4 flex justify-end gap-3">
                    <x-filament::button type="button" color="gray" x-on:click="$dispatch('close-modal', { id: 'sidebar-create-subfolder-modal' })">
                        {{ __('filemanager::messages.cancel') }}
                    </x-filament::button>

                    <x-filament::button type="submit">
                        {{ __('filemanager::messages.create') }}
                    </x-filament::button>
                </div>
            </form>
        </x-filament::modal>
        ...
        <x-filament::modal id="sidebar-rename-modal" width="md">
            <x-slot name="heading">
                {{ __('filemanager::messages.rename_folder') }}
            </x-slot>
            ...
            <form wire:submit="renameItem">
                <x-filament::input.wrapper>
                    <x-filament::input type="text" wire:model="renameItemName" placeholder="{{ __('filemanager::messages.new_name') }}" autofocus/>
                </x-filament::input.wrapper>

                <div class="mt-4 flex justify-end gap-3">
                    <x-filament::button type="button" color="gray" x-on:click="$dispatch('close-modal', { id: 'sidebar-rename-modal' })">
                        {{ __('filemanager::messages.cancel') }}
                    </x-filament::button>
        ...
        <x-filament::modal id="sidebar-move-modal" width="md">
            <x-slot name="heading">
                {{ __('filemanager::messages.move_folder') }}
            </x-slot>

            <x-slot name="description">
                @if($this->itemToMove)
                    {{ __('filemanager::messages.select_destination_for', ['name' => $this->itemToMove->getName()]) }}
                @endif
            </x-slot>

            <div class="max-h-64 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                {{-- Root option --}}
                <button type="button" wire:click="setMoveTarget(null)" class="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 {{ $moveTargetPath === null ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : '' }}">
                ...
                @foreach($this->allFolders as $folder)
                    @if($folder->getIdentifier() !== $itemToMoveId)
                        <button type="button" wire:click="setMoveTarget('{{ $folder->getIdentifier() }}')" class="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 {{ $moveTargetPath === $folder->getIdentifier() ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : '' }}">
                            <x-heroicon-o-folder class="w-4 h-4"/>
            ...
            <div class="mt-4 flex justify-end gap-3">
                <x-filament::button type="button" color="gray" x-on:click="$dispatch('close-modal', { id: 'sidebar-move-modal' })">
                    {{ __('filemanager::messages.cancel') }}
                </x-filament::button>

                <x-filament::button type="button" wire:click="moveItem">
