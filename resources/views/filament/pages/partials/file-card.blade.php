{{-- File/Folder Card for Grid View --}}@php
    $itemId = $item->getIdentifier();
    $itemName = $item->getName();
    $isReadOnly = $isReadOnly ?? false;
    $isSelected = $this->isSelected($itemId);
@endphp
<div wire:key="card-{{ md5($itemId) }}" x-data="{ isDragOver: false }" @if(!$isReadOnly)
    ...
    x-on:dragover.prevent="isDragOver = true" x-on:dragleave="isDragOver = false" x-on:drop.prevent="isDragOver = false; if (draggedItemId && draggedItemId !== @js($itemId)) { $wire.handleDrop(@js($itemId), draggedItemId) }" @endif@endifx-on:click="$wire.handleItemClick(@js($itemId))" class="group relative cursor-pointer rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 transition-all hover:border-primary-500 hover:shadow-lg
        {{ $isSelected ? 'ring-2 ring-primary-500 border-primary-500 bg-primary-50 dark:bg-primary-900/20' : '' }}" @if(!$isReadOnly):class="{ 'border-primary-500 bg-primary-50 dark:bg-primary-900/20': isDragOver }"@endif
>
    {{-- Selection Checkbox --}}
    @if(!$isReadOnly)
        <button type="button" x-on:click.stop="$wire.toggleSelection(@js($itemId), true)" class="absolute left-2 top-2 z-10 flex h-5 w-5 items-center justify-center rounded border transition-all
                {{ $isSelected
        ...
        <div class="flex items-start justify-between gap-2">
            ...
            @if(!$isReadOnly)
                ...
                <x-filament::dropdown placement="bottom-end">
                    <x-slot name="trigger">
                        <button type="button" x-on:click.stop class="p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-gray-100 dark:hover:bg-gray-700 transition-opacity">
                            <x-heroicon-o-ellipsis-vertical class="w-5 h-5"/>
                        </button>
                    </x-slot>

                    <x-filament::dropdown.list>
                        <x-filament::dropdown.list.item icon="heroicon-o-arrow-right-circle" x-on:click.stop="close" wire:click="openMoveDialog('{{ $itemId }}')">
                            {{ __('filemanager::messages.move') }}
                        </x-filament::dropdown.list.item>
                        <x-filament::dropdown.list.item icon="heroicon-o-pencil" x-on:click.stop="close" wire:click="openRenameDialog('{{ $itemId }}')">
                            {{ __('filemanager::messages.rename') }}
                        </x-filament::dropdown.list.item>
                        <x-filament::dropdown.list.item icon="heroicon-o-trash" color="danger" x-on:click.stop="close" wire:click="deleteItem('{{ $itemId }}')" wire:confirm="{{ __('filemanager::messages.confirm_delete') }}">
